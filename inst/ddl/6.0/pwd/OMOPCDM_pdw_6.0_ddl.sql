--pdw CDM DDL Specification for OMOP Common Data Model 6.0
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.PERSON  ( person_id bigint NOT NULL,
...
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.OBSERVATION_PERIOD  (observation_period_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.VISIT_OCCURRENCE  (visit_occurrence_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.VISIT_DETAIL  (visit_detail_id bigint NOT NULL,
			 person_id bigint NOT NULL,
			...
			admitted_from_source_value integer NOT NULL,
			discharge_to_source_value varchar(50) NULL,
			discharge_to_concept_id integer NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.CONDITION_OCCURRENCE  (condition_occurrence_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.DRUG_EXPOSURE  (drug_exposure_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.PROCEDURE_OCCURRENCE  (procedure_occurrence_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.DEVICE_EXPOSURE  (device_exposure_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.MEASUREMENT  (measurement_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.OBSERVATION  (observation_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.NOTE_NLP  (note_nlp_id bigint NOT NULL,
...
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.SPECIMEN  (specimen_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = REPLICATE);
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.SURVEY_CONDUCT  (survey_conduct_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON RANDOM
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.LOCATION  (location_id bigint NOT NULL,
...
WITH (DISTRIBUTION = REPLICATE);
--HINT DISTRIBUTE ON RANDOM
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.LOCATION_HISTORY  (location_id bigint NOT NULL,
...
WITH (DISTRIBUTION = REPLICATE);
--HINT DISTRIBUTE ON RANDOM
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.CARE_SITE  (care_site_id bigint NOT NULL,
...
WITH (DISTRIBUTION = REPLICATE);
--HINT DISTRIBUTE ON RANDOM
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.PROVIDER  (provider_id bigint NOT NULL,
...
WITH (DISTRIBUTION = REPLICATE);
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.PAYER_PLAN_PERIOD  (payer_plan_period_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.COST  (cost_id bigint NOT NULL,
			 person_id bigint NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.DRUG_ERA  (drug_era_id bigint NOT NULL,
			 person_id bigint NOT NULL,
			drug_concept_id integer NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.DOSE_ERA  (dose_era_id bigint NOT NULL,
			 person_id bigint NOT NULL,
			drug_concept_id integer NOT NULL,
...
WITH (DISTRIBUTION = HASH(person_id));
--HINT DISTRIBUTE ON KEY (person_id)
IF XACT_STATE() = 1 COMMIT; CREATE TABLE @cdmDatabaseSchema.CONDITION_ERA  (condition_era_id bigint NOT NULL,
			 person_id bigint NOT NULL,
			condition_concept_id integer NOT NULL,
